...
export const GoalPlanningCardsPage: React.FC<GoalPlanningCardsPageProps> = ({
  ...
  return (
    <div className="min-h-screen bg-[#FBF7F2] text-[#2B2B2B] flex flex-col justify-between selection:bg-[#0F7173] selection:text-white">
      ...
      <main className="container mx-auto px-4 py-8 sm:py-10 flex-1 max-w-6xl">
        {/* HERO TITLE & CALLOUT */}
        <div className="text-center max-w-3xl mx-auto space-y-3 mb-10">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#0F7173]/10 border border-[#0F7173]/20 text-[#0F7173] text-xs font-extrabold">
            ...
            <span>Interactive Goal Sandboxes</span>
          </div>

          <h2 className="text-3xl sm:text-4xl font-extrabold text-[#2B2B2B] tracking-tight leading-tight">
            Select a Financial Goal to Simulate Live
          </h2>

          <p className="text-sm sm:text-base text-[#6B6B6B] leading-relaxed">
            Not sure what to ask the AI? Pick a pre-built template below. Clicking any card opens a pre-filled simulator with realistic targets calculated specifically for your profile.
          </p>

          {/* Occupation Profile Intelligence Badge */}
          <div className="pt-2 flex items-center justify-center gap-2 text-xs font-bold text-[#0F7173]">
            <div className="px-3 py-1.5 rounded-full bg-white border border-slate-200 shadow-sm flex items-center gap-2 text-slate-700">
              <Briefcase className="w-4 h-4 text-[#0F7173]" />
              <span>
                Smart defaults configured for <span className="font-extrabold text-[#0F7173] capitalize">{userOccupation.replace('_', ' ')}</span>
              </span>
